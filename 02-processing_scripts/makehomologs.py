#!/usr/bin/env python
#
# v1.3 2023-06-15 allow multiple fasta files, run sequentially
# v1.2 2023-04-11
...
"""makehomologs.py v1.3  last modified 2023-06-15
...
makehomologs.py -i lactobacillus_all_proteins.all_v_all.tab.gz -f lactobacillus_all_proteins.fasta -p 234 -M 500 -T 4 -o orthologs_v1

  diamond step could be run as, with gzip (better):

diamond-v2.0.13 blastp -q lactobacillus_all_proteins.fasta -d lactobacillus_all_proteins.fasta | gzip > lactobacillus_all_proteins.all_v_all.tab.gz
...
def get_seq_lengths(seq_file_list):
	'''from a fasta file, return a dictionary where protein ID is the key and length is the value'''
	seqlendict = {}
	for seqfile in seq_file_list: # should be list of file names
		print( "# Parsing sequences from {}  {}".format(seqfile, time.asctime() ), file=sys.stderr)
		for seqrec in SeqIO.parse(seqfile,'fasta'):
			seqlendict[seqrec.id] = len(seqrec.seq)
	return seqlendict


def parse_edges(output_dir, blast_hits, min_overlap, min_bitscore, min_cluster_size, species_delimiter, verbose, wayout, seqfiles=None):
	...
	seqlendict = get_seq_lengths(seqfiles)
	...
	with _opentype(blast_hits,'rt') as bh:
		for line in bh:
			# fields:
			blastsplits = line.strip().split('\t') # should split to 12 columns
...
def refine_clusters(outdir, clusterfile, min_clust_size, min_taxa, max_sequences, max_length, seq_file_list, max_homologs, species_delimiter, get_sp_counts, verbose, wayout):
...
def main(argv):
	...
	parser.add_argument('-f','--fasta', nargs='*', help="original fasta sequences, if starting after blast")
	...
	if args.fasta:
		logreport("# {} used as fasta database".format( " ".join(args.fasta) ), wayout )
